...
import styles from './hero-layout-13.module.scss';
import {
  motion,
  useAnimationFrame,
  useMotionValue,
...
// Component
/**
 * HeroLayout13 Component
 * 
 * The `HeroLayout13` component is a page hero component useful to introduce pages.
 * The component contains a primary text content block rendered on a collection of background images.
 * 
 * @component
 * @example
 * <HeroLayout13
 ...
 * />
 * 
 * @param { THeroLayout13Props } props - The properties for configuring the HeroLayout13 component.
 * 
 * @returns { JSX.Element } - The HeroLayout13 component is being returned as a JSX element.
 */
export const HeroLayout13 = ( props: THeroLayout13Props ): JSX.Element => {
  // #region - PROPS
  const {
    actions,
    images,
   ...
   */
  const oddImages: IImage[] = images.filter(( _, index: number) => !(index % 2) );
  /**
   * Stores the list of images that have an index position with an even number.
   */
  const evenImages: IImage[] = images.filter(( _, index: number) => index % 2 );
  /**
  ...
  const generateImageGroup = ( type: 'odd' | 'even' ): JSX.Element => (
    <motion.div
      style={{
        translateY: ( type === 'even' ) ? translateX.negative : translateX.positive
      }}
      className={`
      ...
      `}
    >
      { [ ...Array( type === 'even' ? ( windowSize >= breakpoints.sm ) ? 2 : 3 : 3 ) ].map(( _, index: number ) => (
        <div
